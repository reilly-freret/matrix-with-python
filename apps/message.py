...
class MessageApp(App):
    ...
    def __init__(
        self, image_setter, data_refresh_rate=10, render_refresh_rate=1
    ) -> None:
        ...
        self.mqttc.on_connect = (
            lambda client, userdata, flags, reason_code, properties: self.on_connect(
                client, userdata, flags, reason_code, properties
            )
        )
        self.mqttc.on_message = lambda client, userdata, msg: self.on_message(
            client, userdata, msg
        )
        ...
        self.mqttc.connect(
            "d7ad67d5cbb14999894675743d50e0e6.s1.eu.hivemq.cloud", 8883, 60
        )
