...
class SpotifyApp(App):
    ...
    def __get_new_access_token__(self) -> str:
        ...
        form_data = {"grant_type": "refresh_token", "refresh_token": rt}
        access_response = post(
            "https://accounts.spotify.com/api/token", headers=headers, data=form_data
        )
    ...
    def __set_new_access_token_inplace__(self):
        ...
        self.__api_headers__ = {"Authorization": f"Bearer {access_token}"}
    ...
    def __data_update__(self):
        ...
        try:
            ...
            pass
        if (
            (self.__data__ is not None)
            and ("item" in self.__data__)
            and (new_album_id != self.__last_album_id__)
        ):
            ...
            if new_album_id in self.__album_art_cache__:
                ...
                a_url = next(
                    image["url"]
                    for image in self.__data__["item"]["album"]["images"]
                    if image["width"] == 64
                )
                logger.debug(a_url)
    ...
    def __render_update__(self):
        ...
        if self.__data__ is None:
            ...
            self.__canvas__.paste(self.__album_art__, (1, self.__top_padding__))

            if self.__data__["item"]["id"] != self.__last_song_id__:
                self.__title_scroller__.update_text(self.__data__["item"]["name"])
            ...
            draw.rectangle(
                (6, progress_bar_y, 62, progress_bar_y), fill=self.__DARK_GRAY__
            )
            draw.rectangle(
                (6, progress_bar_y, 6 + progress_as_pixel, progress_bar_y),
                fill=highlight_color,
            )
            draw.rectangle(
                (
                    6 + progress_as_pixel,
                    self.__top_padding__ + 20,
                    6 + progress_as_pixel,
                    self.__top_padding__ + 22,
                ),
                fill=highlight_color,
            )
            self.__canvas__.paste(play_or_pause, (1, self.__top_padding__ + 19))
