...
class Scroller():
    def __init__(self, canvas: Image.Image, x: int, y: int, width: int, text: str) -> None:
        ...
        self.__sub_canvas__ = Image.new('RGB', (self.__width__, self.__text_height__), (0,0,0))
        self.__PADDING__ = 20
        self.__WAIT_THRES__ = 30
    ...
    def incr(self):
        ...
        self.__sub_canvas__.paste((0,0,0), (0, 0, self.__sub_canvas__.size[0], self.__sub_canvas__.size[1]))
        ...
        if self.__text_width__ < self.__width__:
            draw.text((self.__current_offset__, 0), self.__text__, fill=self.__color__, font=self.__font__)
            self.__canvas__.paste(self.__sub_canvas__, (self.__x__, self.__y__))
            return

        # draw text with offset
        draw.text((self.__current_offset__, 0), self.__text__, fill=self.__color__, font=self.__font__)
        draw.text((self.__current_offset__ + self.__text_width__ + self.__PADDING__, 0), self.__text__, fill=self.__color__, font=self.__font__)
